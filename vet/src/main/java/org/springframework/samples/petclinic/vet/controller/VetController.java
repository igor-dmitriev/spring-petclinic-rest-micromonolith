...
public class VetController {
  ...
  public ResponseEntity<Vet> addVet(@RequestBody @Valid Vet vet, BindingResult bindingResult, UriComponentsBuilder ucBuilder) {
    ...
    headers.setLocation(ucBuilder.path("/api/vet/{id}").buildAndExpand(vet.getId()).toUri());
  ...
  public ResponseEntity<Vet> updateVet(@PathVariable("vetId") Integer vetId, @RequestBody @Valid Vet vet, BindingResult bindingResult) {
    BindingErrorsResponse errors = new BindingErrorsResponse();
    HttpHeaders headers = new HttpHeaders();
    if (bindingResult.hasErrors() || (vet == null)) {
      errors.addAllErrors(bindingResult);
